...
fn test_big5_encode() {
  ...
  assert_eq!(
    BIG5_2003.encode(&string, EncoderTrap::Ignore),
    Ok(vec![0xC6, 0x7B, 0xB4, 0x65, 0xB6, 0xBA])
  );
}
...
fn test_big5_decode() {
  ...
  assert_eq!(
    BIG5_2003.decode(&arr, DecoderTrap::Ignore),
    Ok(String::from("鬱悶飯"))
  );
}
...
fn test_big5_decode_2() {
  let arr: [u8; 16] = [
    0xB4, 0xF6, 0xB7, 0x6E, 0xB2, 0xF8, 0xAA, 0xBA, 0xAB, 0xD5, 0xA9, 0x60, 0xA6, 0x50, 0xBE, 0xC7,
  ];
  assert_eq!(
    BIG5_2003.decode(&arr, DecoderTrap::Ignore),
    Ok(String::from("湯搖莊的幽奈同學"))
  );
}
